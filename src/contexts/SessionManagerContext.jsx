...
export const SessionManagerProvider = ({ children }) => {
  ...
  const removeActiveSession = useCallback((projectName, sessionId) => {
    setActiveProjectSessions(prev => {
      const newMap = new Map(prev);
      ...
      console.log(`[SessionManager] Removed active session ${sessionId} from project ${projectName}`);
      return newMap;
    });
    // Clear current active session if it matches
    if (currentActiveSession?.projectName === projectName && currentActiveSession?.sessionId === sessionId) {
  ...
  const addProcessingSession = useCallback((projectName, sessionId) => {
    setProcessingProjectSessions(prev => {
      ...
      newMap.get(projectName).add(sessionId);
      console.log(`[SessionManager] Added processing session ${sessionId} to project ${projectName}`);
      return newMap;
    });
  }, []);
  ...
  const removeProcessingSession = useCallback((projectName, sessionId) => {
    setProcessingProjectSessions(prev => {
      ...
      console.log(`[SessionManager] Removed processing session ${sessionId} from project ${projectName}`);
      return newMap;
    });
  }, []);

  // Check if a specific session is active in its project
  const isSessionActive = useCallback((projectName, sessionId) => {
  ...
  const isSessionProcessing = useCallback((projectName, sessionId) => {
