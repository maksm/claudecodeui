...
const mockProjects = [
  ...
  },
  {
    ...
    sessions: [
      { id: 'session-3', name: 'Bug Fixing', createdAt: '2024-01-04T00:00:00Z' }
    ]
  }
];
...
export const handlers = [
  ...
  http.post('/api/projects', (req, res, ctx) => {
    ...
    if (!name || !path) {
      return res(
        ctx.status(400),
        ctx.json({ error: 'Name and path are required' })
      );
    }
  ...
  http.post('/api/projects/:projectId/sessions', (req, res, ctx) => {
    ...
    if (!name) {
      return res(
        ctx.status(400),
        ctx.json({ error: 'Session name is required' })
      );
    }
  ...
  http.post('/api/tasks', (req, res, ctx) => {
    ...
    if (!title || !projectId) {
      return res(
        ctx.status(400),
        ctx.json({ error: 'Title and projectId are required' })
      );
    }
  ...
  http.post('/api/git/commit/:projectPath', (req, res, ctx) => {
    ...
    if (!message) {
      return res(
        ctx.status(400),
        ctx.json({ error: 'Commit message is required' })
      );
    }
  ...
  http.post('/api/claude/stream', (req, res, ctx) => {
    ...
    return res(
      ...
      ctx.body(`data: {"type": "content", "text": "Mock stream response for: ${message}"}\n\ndata: [DONE]\n`)
    );
  }),
