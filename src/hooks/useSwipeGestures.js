...
export const useSwipeGestures = (onSwipeLeft, onSwipeRight, onSwipeUp, onSwipeDown, config = {}) => {
  ...
  const executeSwipeCallback = useCallback((direction) => {
  ...
  const handleStart = useCallback((clientX, clientY, event) => {
  ...
  const handleMove = useCallback((clientX, clientY, event) => {
    ...
    const restrainedX = Math.max(-mergedConfig.restraint, Math.min(mergedConfig.restraint, deltaX));
    const restrainedY = Math.max(-mergedConfig.restraint, Math.min(mergedConfig.restraint, deltaY));
  ...
  const handleEnd = useCallback((event) => {
    ...
    const isFastSwipe = velocityX > mergedConfig.velocityThreshold || velocityY > mergedConfig.velocityThreshold;
    ...
    setIsDragging(false);
    setIsSwiping(false);
  }, [isDragging, mergedConfig, calculateSwipeMetrics, determineSwipeDirection, executeSwipeCallback, api]);
  ...
  const AnimatedComponent = useCallback(({ children, className = '', style = {}, ...props }) => (
    <animated.div
      ref={elementRef}
